...
def visualize_maze(maze: list[list[str]], start: tuple[int, int], goals: list[tuple[int, int]], path: list[tuple[int, int]], title: str, cost: int, ne: int, md: int, mf: int) -> None:
    ...
    Args:
        ...
        cost: Path cost
        ne: Number of nodes expanded
        md: Maximum depth traversed
        mf: Maximum fringe size
    """
    rows = len(maze)
    cols = len(maze[0])

    grid = np.zeros((rows, cols))
    plt.figure(figsize=(12 if rows > 25 else 6, 6))
    ...
    plt.scatter(start[1], start[0], color='orange', label='Start', linewidth=2)
    
    for goal in goals:
        # Add a scatter plot (circle) with color green in place of end node
        plt.scatter(goal[1], goal[0], color='green', label='Goal', linewidth=2)

    # Plot the entire path as a yellow thin line
    if path:
        path_x, path_y = zip(*path)  # Unzip path into x and y coordinates
        plt.plot(path_y, path_x, color='yellow', linewidth=2, label='Path')
    ...
    plt.figtext(0.9, 0.1, f'Maximum depth: {md}', ha='right', va='bottom', fontsize=12)
    plt.figtext(0.9, 0.05, f'Maximum fringe size: {mf}', ha='right', va='bottom', fontsize=12)
