    ...
    for event in events:
      if event.type == pygame.QUIT:
        ...
        if self.isabletomove():
          self.movepiece()
          if gv.gameplay_option == 0 and self.isgoalstate():
            self.status = 3
  ...
  def draw(self, scene):
    ...
    if self.status == 1:
        ...
        if self.boardmatrix[self.ori_x][self.ori_y] == 1:
            ...
            # right down
            if y2 <= len(self.boardmatrix[0])-1 and self.boardmatrix[x2][y2] != 1:
                self.scene.blit(self.greenoutline,
                                  (self.sizeofcell * y2 + 4, self.sizeofcell * x2 + 4))
            # down
            if x3 <= len(self.boardmatrix)-1 and self.boardmatrix[x3][y3] == 0:
                self.scene.blit(self.greenoutline,
        ...
        if self.boardmatrix[self.ori_x][self.ori_y] == 2:
            ...
            if y1 >= 0 and self.boardmatrix[x1][y1] != 2:
                self.scene.blit(self.whiteoutline,
                                  (self.sizeofcell * y1 + 4, self.sizeofcell * x1 + 4))
            # right up
            if y2 <= len(self.boardmatrix[0])-1 and self.boardmatrix[x2][y2] != 2:
                self.scene.blit(self.whiteoutline,
                                  (self.sizeofcell * y2 + 4, self.sizeofcell * x2 + 4))
            # up
  ...
  def isgoalstate(self):
    if gv.rule_index == 0:
      ...
      if count == 3:
        return True
      count = 0
      for i in self.boardmatrix[len(self.boardmatrix)-1]:
        if i == 1:
          count += 1
      if count == 3:
        return True
